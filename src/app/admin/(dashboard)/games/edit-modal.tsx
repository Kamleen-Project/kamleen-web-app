...
import { updateGameEvent } from "@/app/actions/games";
...
export function AdminEditGameModal({ gameEvent, coupons }: Props) {
    const router = useRouter();
    const [open, setOpen] = useState(false);
    const [isLoading, setIsLoading] = useState(false);
    const [dateRange, setDateRange] = useState<CalendarDateRange | undefined>({
        from: new Date(gameEvent.startDate),
        to: new Date(gameEvent.endDate),
    });
    const [prizes, setPrizes] = useState<Prize[]>(
        gameEvent.prizes.map((p) => ({
            id: p.id,
            couponId: p.couponId,
    ...
    async function onSubmit(formData: FormData) {
        ...
        try {
            ...
            const result = await updateGameEvent(gameEvent.id, {
                type: formData.get("type") as GameType,
                title: formData.get("title") as string,
                description: formData.get("description") as string || undefined,
                startDate: dateRange.from,
                endDate: dateRange.to,
                isActive: gameEvent.isActive,
                prizes: prizesData,
            });
            ...
            if (result.success) {
                ...
                alert(result.error || "Failed to update game event");
            }
        } catch (error) {
            alert("Failed to update game event");
        } finally {
            setIsLoading(false);
        }
    }
    ...
    return (
        <Dialog open={open} onOpenChange={setOpen}>
            <DialogTrigger asChild>
                <CtaIconButton color="whiteBorder" ariaLabel="Edit" size="sm">
            ...
            <DialogContent className="sm:max-w-4xl max-h-[90vh] overflow-y-auto">
                <DialogHeader>
                    ...
                    <DialogDescription>
                        Update the game event configuration and prizes.
                    </DialogDescription>
                </DialogHeader>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div className="space-y-4">
                            ...
                            <TextareaField
                                ...
                                rows={3}
                                defaultValue={gameEvent.description || ""}
                            />
