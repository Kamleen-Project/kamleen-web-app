...
export function FormLabel({ className, ...props }: FormLabelProps) {
  ...
  return (
    <label
      htmlFor={field.id}
      className={cn("mb-2 block text-sm font-medium leading-tight text-foreground", className)}
      {...props}
    />
  )
}
...
export function FormInput({ className, ...props }: FormInputProps) {
  const field = useFieldContext()
  return <input id={field.id} className={cn("flex h-11 w-full rounded-md border border-input bg-background px-4 text-sm shadow-sm transition focus-visible:border-ring focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring/30 disabled:cursor-not-allowed disabled:opacity-50", className)} {...props} />
}
...
export function FormTextarea({ className, ...props }: FormTextareaProps) {
  const field = useFieldContext()
  return <textarea id={field.id} className={cn("min-h-[120px] w-full rounded-md border border-input bg-background px-4 py-3 text-sm shadow-sm transition focus-visible:border-ring focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring/30 disabled:cursor-not-allowed disabled:opacity-50", className)} {...props} />
}
...
export function FormSelect({ className, children, ...props }: FormSelectProps) {
  ...
  return (
    <select
      id={field.id}
      className={cn(
        "flex h-11 w-full rounded-md border border-input bg-background px-4 text-sm shadow-sm transition focus-visible:border-ring focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring/30 disabled:cursor-not-allowed disabled:opacity-50",
        className
      )}
      {...props}
    >
